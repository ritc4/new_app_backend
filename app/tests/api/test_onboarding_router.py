...
from app.core.dependencies.onboarding import get_onboarding_service
from app.main import app
from app.schemas.base import UserRole  # Импортируем для правильного payload ролей
from app.schemas.onboarding import (
    BankType,
    BankWebhookPayloadResponse,
    CancelCurrentApplicationResponse,
...
# =====================================================================
# 1. СТРОГИЙ ИСПРАВЛЕННЫЙ МОК-СЕРВИС (Синхронизирован со всеми типами)
# =====================================================================
class MockOnboardingService:
    async def create_application(self, _user_id: int, _data: object) -> OnboardingLinkResponse:
        return OnboardingLinkResponse(status="ok", message="создано", link="https://tinkoff.ru")

    async def process_bank_webhook(self, _payload: object) -> BankWebhookPayloadResponse:
        return BankWebhookPayloadResponse(status="ok", message="обновлено")

    # ИСПРАВЛЕНО: Добавлен 4-й аргумент _user_uuid для соответствия новому контракту сервиса
    async def get_onboarding_upload_url(
        self, _user_id: int, _user_uuid: str, _file_type: str, _content_type: str
    ) -> OnboardingUploadResponse:
        # ИСПРАВЛЕНО ДЛЯ PYDANTIC V2: Структура upload_data строго содержит url и fields
        return OnboardingUploadResponse(
            upload_data={"url": "https://yandexcloud.net", "fields": {"key": "value"}}, file_url="https://s3.com"
        )

    async def submit_survey(self, _user_id: int, _data: dict) -> SubmitSurveyResponse:
        return SubmitSurveyResponse(status="success", message="отправлено")

    async def cancel_current_application(self, _user_id: int) -> CancelCurrentApplicationResponse:
        return CancelCurrentApplicationResponse(status="success", message="отменена")


@pytest.fixture(autouse=True)
def override_onboarding_service():
    """Автоматически подменяет реальный сервис онбординга на заглушку во всех тестах."""
    app.dependency_overrides[get_onboarding_service] = lambda: MockOnboardingService()
    yield
    if get_onboarding_service in app.dependency_overrides:
        del app.dependency_overrides[get_onboarding_service]
...
async def test_start_onboarding(client, user_token):
    headers = {"Authorization": f"Bearer {user_token}"}
    payload = {
        "target_role": UserRole.SUPPLIER.value,  # ИСПРАВЛЕНО: Строгое значение из Enum
        "bank": BankType.T_BANK.value,
    }
    response = await client.post(f"{BASE_URL}/start", json=payload, headers=headers)
    assert response.status_code == 200
...
async def test_get_upload_link(client, user_token):
    ...
    assert "file_url" in data
    assert "upload_data" in data
    # Дополнительная проверка структуры вложенного контракта Яндекса:
    assert "url" in data["upload_data"]
    assert "fields" in data["upload_data"]
...
async def test_submit_survey(client, user_token):
    ...
    # Payload, который удовлетворяет ВСЕМ вашим enterprise-валидаторам SupplierSurvey
    payload = {
        "role_type": "supplier",  # Добавили обязательный дискриминатор
        "car_brand": "Kia",
        "car_model": "Rio",
        "car_year": current_year - 2,
        "car_number": "А777АА77",
        "car_color": "Белый",
        "vin_number": "1234567890ABCDEFG",
        "license_number": "9901123456",
        "license_expiry_date": str(date(current_year + 5, 1, 1)),
        "license_country": "RU",
        "experience_years": 5,
        "photo_selfie": "id_1",
        "photo_car_side": "id_2",
